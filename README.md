...
AgendAI 会小纪面向需要沉淀会议记录并持续跟进会议行动项的个人和小团队。会议数据默认保存在本机；你可以接入自己的兼容 OpenAI API 的模型服务完成转写、纪要和分析，也可以通过 Claude Code MCP 将会后待办匹配并交接到禅道。
...
- 个人工作台：把会议、会议纪要、AI 分析和 Agent 生成的待办统一到按日期跟踪的工作台。
...
| 模块 | 能力 |
| --- | --- |
| 会议与工作台 | 新建、暂停、恢复、结束、归档会议；在个人工作台按季度、日期和人员查看会议与任务。 |
...
| AI 分析与 Agent | 独立生成分析结论和结构化待办，不覆盖正式会议纪要；待办和未决事项可匹配禅道并交接。 |
| 工作项闭环 | 在“待办任务池”和“工作台”统一管理会议行动项、AI 分析任务、Agent 待办和手动任务；支持负责人、计划日期、优先级、标签、状态和完成备注。 |
| 状态与排期 | 五种任务状态支持任意直接切换；只有负责人和截止日期确认完整的任务才进入工作台日历，待确认任务回到待办任务池。 |
| 外部系统 | 在“设置 > 外部系统 > 禅道”配置 MCP 地址、传输类型和账号，支持内网或公网 HTTPS 部署，并提供权限测试与诊断。 |

## 界面预览

当前版本的界面由“工作台”“待办任务池”和“设置”组成。下面的截图来自 v0.1.9；其中负责人姓名字段已做脱敏处理。

<p align="center">
  <img src="assets/screenshots/workbench.png" alt="AgendAI v0.1.9 个人工作台周视图，负责人信息已脱敏" width="1100">
</p>

待办任务池集中处理负责人、计划日期和截止日期；确认后的任务会进入工作台日历。

<p align="center">
  <img src="assets/screenshots/work-item-pool.png" alt="AgendAI v0.1.9 待办任务池，负责人信息已脱敏" width="1100">
</p>
...
6. 打开“待办任务池”，在一级列表中补充负责人、计划开始日期和截止日期；确认后任务会进入“工作台”日历。也可以点击“新建任务”创建个人任务。
7. 在工作台任务卡或任务详情中维护计划起止日期、优先级、标签和完成备注；任务状态可直接切换，不要求按固定顺序流转。
8. 在“设置 > 外部系统 > 禅道”配置 MCP 后，可在待办或未决事项上执行匹配、重新生成、单条交接或批量交接。
9. 确认内容后导出 Markdown 或 HTML。

## 禅道交接

禅道地址是应用配置项，不绑定某个固定 IP。你可以填写内网地址、局域网域名或公网 HTTPS 域名，例如 `https://zentao.example.com`，同时配置 Claude Code MCP endpoint 和传输类型。

会后待办通过 Claude Code 调用禅道 MCP 查询项目、执行、人员和已有任务，再生成匹配结果；支持单条交接、一键交接和重新生成。应用优先复用 Claude Code 用户级 `zentao` 配置；只有明确提供 `ZENTAO_MCP_TOKEN` 时才生成会话级临时配置，Token 不写入数据库、项目文件或日志。

MCP 权限测试只调用禅道只读工具，不会创建、修改或删除任务。连接失败、未认证、超时、非法 JSON 和进程退出会显示可读诊断，并支持查看执行阶段、配置来源、CLI 路径和脱敏后的错误详情。

建议为禅道创建专用个人 API Token，并只授予所需的最小权限。公网部署时请使用 HTTPS，并通过网络访问控制限制禅道管理接口。
...
安装脚本会构建正式版 `v0.1.9`、创建 DMG、挂载并校验签名，然后安装到 `/Applications`。也可以直接下载 [v0.1.9 Release](https://github.com/leoyoul/AgendAI/releases/tag/v0.1.9) 中的 `AgendAI-v0.1.9-macOS-universal.dmg`，将左侧的“会小纪”拖到右侧的“Applications”。首次打开后，请在应用内完成模型配置。
...
“待确认、待开始、进行中、已完成、已取消”之间不再有强制的线性流转限制。切换为“待确认”时，任务仍会按排期规则回到待办任务池；负责人、标题和日期等基础校验仍然有效。
...
assets/screenshots/ README 使用的脱敏界面截图
```
