...
struct MeetingMinutesGeneratorTests {
    ...
    func rendersHighFidelityMarkdown() {
        ...
        #expect(markdown.contains("## 一、会议概览"))
        #expect(markdown.contains("## 二、会议摘要"))
        #expect(markdown.contains("## 三、会议议程与讨论"))
        #expect(markdown.contains("## 四、会议结论"))
        #expect(markdown.contains("## 五、未决事项"))
        #expect(markdown.contains("## 六、已确定待办"))
        #expect(markdown.contains("## 七、时间节点与归档资料"))
        #expect(markdown.contains("00:01:20-00:02:50"))
        #expect(markdown.contains("主要观点"))
        #expect(markdown.contains("形成结果"))
        #expect(markdown.contains("**状态：** 暂定方向"))
        #expect(markdown.contains("方案初稿"))
        #expect(markdown.contains("保单范围尚未完整确认"))
        #expect(markdown.contains("核对正式文件"))
        #expect(markdown.contains("取得完整保单"))
        #expect(markdown.contains("责任人 | 时间节点"))
        #expect(markdown.contains("时间节点状态"))
    ...
    func rendersHighFidelityHTML() {
        ...
        #expect(html.contains("三、会议议程与讨论"))
        #expect(html.contains("class=\"topic-block\""))
        #expect(html.contains("00:01:20-00:02:50"))
        #expect(html.contains("&lt;一&gt;"))
        #expect(html.contains("&amp; 约束"))
        #expect(html.contains("@media (max-width:720px)"))
        #expect(html.contains("width:min(100%, 980px)"))
